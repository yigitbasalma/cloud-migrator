...
def start():
    ...
    for vpc in CLIENT.describe_vpcs()["Vpcs"]:
        _vpc_object = RESOURCE.Vpc(vpc["VpcId"])
        ...
        for subnet in _vpc_object.subnets.all():
            _subnet_object = RESOURCE.Subnet(subnet.id)
        ...
        for route_table in _vpc_object.route_tables.all():
            _route_table_object = RESOURCE.RouteTable(route_table.id)
            _vpc["route_tables"].append(dict(id=_route_table_object.route_table_id,
                                             subnets=[
                                                 i.get("SubnetId") for i in _route_table_object.associations_attribute
                                             ],
        ...
        for internet_gateway in _vpc_object.internet_gateways.all():
            _internet_gateway_object = RESOURCE.InternetGateway(internet_gateway.id)
            _vpc["internet_gatways"].append(dict(id=internet_gateway.id, tags=internet_gateway.tags))
        RESULT.append(_vpc)

    LOGGER.info(f"VPC info {RESULT}")
